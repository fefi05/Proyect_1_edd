...
public class Lista<T> {
    ...
    public int obtenerIndice(Nodo<T> Busqueda) {
        if (this.esVacio()) {
            return -1;
        } else {
            Nodo<T> actual = this.pPrim;
            ...
            while (actual != Busqueda && encontrado == false) {
                actual = actual.getpSig();
                indice++;
                if (actual == Busqueda) {
                    encontrado = true;
    ...
    public Nodo<T> obtenerNodoPorIndice(int indice) {
        ...
        Nodo<T> actual = getpPrim();
        int contador = 0;

        while (actual != null) {
            if (contador == indice) {
                return actual; // Nodo encontrado en el índice especificado
            }
